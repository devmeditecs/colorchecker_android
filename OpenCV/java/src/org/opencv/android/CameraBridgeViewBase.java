...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    protected Size calculateCameraFrameSize(List<?> supportedSizes, ListItemAccessor accessor, int surfaceWidth, int surfaceHeight) {
        ...
        double aspect_ratio = maxAllowedWidth / (double) maxAllowedHeight;

        Collections.sort(supportedSizes, (d1, d2) -> {
            return (accessor.getWidth(d2) * accessor.getHeight(d2)) - (accessor.getWidth(d1) * accessor.getHeight(d1));
        });
        for (Object size : supportedSizes) {
            ...
            if (Math.abs(aspect_ratio - aspect) < Math.abs(aspect_ratio - calcAspect)) {
                calcWidth = width;
                calcHeight = height;
                calcAspect = aspect;
            }
